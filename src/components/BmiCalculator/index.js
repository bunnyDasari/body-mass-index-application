...
const BmiCalculator = () => {
  const storedHeight = JSON.parse(localStorage.getItem('hight'))
  const storedWeight = JSON.parse(localStorage.getItem('weight'))
  const [height, setHeight] = useState(storedHeight !== null ? setHeight : 170)
  const [weight, setWeight] = useState(storedWeight !== null ? setWeight : 60)
